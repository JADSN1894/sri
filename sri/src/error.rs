...
use std::{
  error::Error as StdError, io::Error as StdIoErro, num::ParseIntError, rc::Rc,
};
use std::{
  fmt::{self, Display},
  num::TryFromIntError,
...
use super::{algorithm::AlgorithmSri, constants::MAX_OF_ARGS};
...
impl Display for SriErrorTypes {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match self {
      ...
      Self::OsString(error) => write!(f, "{error:?}"),
...
impl From<SriErrorTypes> for SriErrorCode {
  fn from(value: SriErrorTypes) -> Self {
    match value {
      ...
      SriErrorTypes::OsString(_) => 8.into(),
...
impl From<OsString> for SriError {
  fn from(value: OsString) -> Self {
    SriErrorTypes::OsString(value).into()
  }
}
